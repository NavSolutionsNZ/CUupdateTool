/*
 * build_manual.js — regenerate triage/USER_MANUAL.docx from triage/USER_MANUAL.md.
 *
...
// {{VERSION}} text in it.
const VERSION = (() => {
  const initPy = path.join(DOCS, '__init__.py');
...
const cover = [
  body[0],
  new Paragraph({ children: [new TextRun({ text: 'incadea / BC-NAV v14 CU upgrade triage & pipeline', size: 24, color: '595959' })], spacing: { after: 80 } }),
  new Paragraph({ children: [new TextRun({ text: 'User Manual', size: 22, color: '595959' })], spacing: { after: 400 } }),
...
const doc = new Document({
  ...
  sections: [{
    ...
    headers: { default: new Header({ children: [new Paragraph({ alignment: AlignmentType.RIGHT, children: [new TextRun({ text: 'CU Triage & Pipeline — User Manual', size: 16, color: '808080' })] })] }) },
